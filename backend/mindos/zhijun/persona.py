...
# ---------------------------------------------------------------------------
# PRD V2 5.5「接住与边界」。判定在 disclosure.py，纯本地、不依赖模型自觉；
# 这两段是判定命中后加到系统提示词里的行为约束。
# ---------------------------------------------------------------------------

RECEIVE_INSTRUCTION = """本轮用户说了一件重的事。这一轮的任务是接住，不是解决。

不要追问细节，不要给建议，不要列选项，不要转向解决方案。也不要说「我理解你的感受」这类替对方定性的话——你没有感受，说了会让人觉得被敷衍。

可以做的只有三件，选一件就够：承认这件事的分量；给出一条与之相关的、用户自己说过的旧话；或者只说很少的话，把余地留给他。

是否继续、往哪儿走，由用户下一轮决定。本轮不会记下任何东西，也不要提起你记了什么。"""


def crisis_instruction(resources: dict | None) -> str:
    """危机处置。求助渠道由调用方从本地清单读出后传入，模型不得自行编造号码。"""
    lines = []
    if resources:
        for item in resources.get("lines", []):
            hours = f"（{item['hours']}）" if item.get("hours") else ""
            lines.append(f"- {item['name']}：{item['contact']}{hours}")
        if resources.get("inPerson"):
            lines.append(f"- {resources['inPerson']}")
    listing = ("\n".join(lines) if lines
               else "（本机没有可用的求助渠道清单。不要编造号码，改为建议他联系当地的急救或医疗机构。）")
    urgent = (resources or {}).get("always", "如果有立即的危险，请联系当地急救。")

    return f"""本轮用户说出了与自我伤害或严重心理困扰有关的话。按下面这样做，不要偏离。

先接住，承认这句话的分量。然后明确说明：你不是心理咨询，也不能替代专业帮助。

把下面这些原样给他，不要改写，也不要补充任何其它号码：

{urgent}
{listing}

如果他以前提到过某个具体的人，建议他去找那个人谈。

不要做这些：追问细节、评估严重程度、给出诊断或治疗建议、让对话继续往这个方向深入、说「我理解你的感受」这类扮演情绪的话、把这件事和他的工作或判断扯在一起。

这段对话不会被记录成任何理解，也不要提起记忆这件事。"""
