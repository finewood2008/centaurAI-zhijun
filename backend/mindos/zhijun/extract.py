    ...
    "properties": {
        "claims": {
            ...
            "items": {
                ...
                "required": ["section", "layer", "predicate", "subject", "object", "content", "quote", "confidence", "scope_hint", "privacy_hint"],
...
输出格式：{"claims":[{"section":"...","layer":"...","predicate":"...","subject":"me","object":null,"content":"...","quote":"...","confidence":0.0-1.0,"scope_hint":"long_term|context_only|unknown","privacy_hint":"private|sensitive"}],"entities":[{"name":"...","type":"person","aliases":[]}]}
只输出 JSON。"""
...
def build_request(user_text: str, prev_assistant: str | None, known_entities: list[str], *, debug: dict | None = None) -> ChatRequest:
    context_lines = []
    if prev_assistant:
...
def validate(raw: dict, *, user_text: str, prev_assistant: str | None) -> list[ValidatedClaim]:
    ...
    for item in items:
        ...
        if layer == "self_declared" and not (_FIRST_PERSON_RE.search(quote) or prev_asked):
            layer = "hypothesis"
            downgraded = True
        if layer == "aspirational" and not _ASPIRATION_RE.search(quote):
    ...
    for claim in valid:
        ...
        try:
            result = store.create_claim(
                ...
                note="用户原话，抽取校验通过" if auto_confirm else "模型抽取的候选",
            )
        except OntologyConflictError:
            continue
...
def run_extraction(
    *,
    ...
    request = build_request(user_text, prev_assistant, known, debug=debug)
