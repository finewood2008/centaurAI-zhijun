...
zhijun_status.provider_status = lambda: {'provider': 'synthetic', 'model': 'revisit-fixture', 'external': True, 'configured': True, 'error': None}
service = routing.service_info(online)['id']
RoutingStore(onto).set_policy('global', enabled=True, service=service, service_name='合成服务', include_files=False,
...
decision = GrowthStore.instance().create_decision({'title': '合成：小范围合作试点', 'context': '先核对双方的合作方式',
    ...
    'expectedOutcome': '两周内完成一次交付', 'reviewAt': '2026-01-01T00:00:00Z', 'relatedEntityIds': [],
...
# A result, a future choice and an old manually selected chat cover selective inclusion.
store = GrowthStore.instance()
payload = {'title': '合成：一次不同意见的沟通', 'context': '核对沟通方式', 'options': ['书面说明', '当面交流'],
    'choice': '当面交流', 'rationale': '留出双方澄清的机会', 'confidence': 60, 'expectedOutcome': '确认各自的关注点',
    'reviewAt': None, 'relatedEntityIds': [], 'evidenceRefs': []}
result_decision = store.create_decision(payload)
store.record_outcome(result_decision['id'], {'result': '合成结果：双方明确了各自的关注点。', 'notes': '', 'evidenceRefs': []})
future = store.create_decision({**payload, 'title': '合成：未来再看的选择', 'reviewAt': '2099-01-01T00:00:00Z'})
archived = convs.create_conversation(title='合成：已经归档的经历')
convs.append_message(archived['id'], 'user', '合成原话：那次经历已告一段落。')
# Use the public store metadata operation; archived chats remain read-only in the UI.
convs.update_metadata(archived['id'], expected_revision=0, status='archived')
for i in range(42):
    filler = convs.create_conversation(title=f'合成：日常查资料 {i + 1}')
    convs.append_message(filler['id'], 'user', '这只是合成的普通问答，不应自动进入回看。')
...
@app.get('/__fixture')
def info():
    return {'synthetic': True, 'decisionId': decision['id'], 'sourceId': source['id'], 'conversationId': ordinary['id'], 'futureId': future['id'], 'resultId': result_decision['id'], 'archivedId': archived['id'], 'requests': len(local.requests) + len(online.requests)}
...
if __name__ == '__main__':
    uvicorn.run(app, host='127.0.0.1', port=int(os.environ.get('REVISIT_PORT', '8777')), log_level='warning')
