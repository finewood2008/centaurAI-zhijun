...
**编号用 Q 前缀，不用 P**：PRD 第 12 节的 P0–P4 是分期名（P1=内观、P2=输出口），两套编号共用字母会读错。

| 编号 | 前提 |
|---|---|
| Q1 | 独立软件，一个数据文件夹，无账号，可备份、可拷走、可删除 |
| Q2 | 资料（导入文档）只作为理解你的证据，不做文档问答；搜索框搜的是「你的理解与判断」，资料以出处形式出现 |
| Q3 | MCP 按分区默认可读：我是谁 / 原则 / 做法 / 方向 默认可读；重要的人 / 正在做的事 默认关；**心里的事 / 我眼中的我、敏感与受限永不外发且无开关** |
| Q4 | 目标收敛到 6 类一等对象、约 15 张表、约 60 条路由 |
| Q5 | **知君是让用户直面内心的产品，不是提效工具**（2026-09-21 定位修正）。数据层必须记得下内心，并且必须保证内心不会流向其他 AI |
...
| **C7** | **内心不外流**：`burdens` 与 `self_view` 永不进入 `可带走()` 的任何通道（MCP、上下文包、USER.md 投影、导出包的 `takeaway` 范围），没有开关。**知君自己调用的模型与用户自己的全量备份不在此列**，理由见 6.3 | 原则 7；对应 MCP 契约 I6 |
...
| `docs/product/PRODUCT.md` | 面向界面的产品定义，已于 2026-09-21 按新定位同步重写。本文**补充**它的数据层部分；两者冲突时以 PRD V2 与本文为准 |
...
| `section` | 有，6 值 | **8 值**（4.4 新增 `burdens` / `self_view`） | 出门规则的分区默认依赖它；新增两值在 `可带走()` 上是硬排除（6.4） |
...
`PREDICATES`（`:61-68`）现有 21 个，**目标态 27 个**，**按分区封闭，越界整条丢弃**（校验 `:804-806`）：
...
**八个分区的界面名，目标态取这一套，全产品唯一**：我是谁 / 重要的人 / 正在做的事 / 我的原则 / 相处方式 / 我要去哪 / 心里的事 / 我眼中的我。今天 `SECTION_TITLES:78-85` 用的是更短的「我的人 / 我的事 / 我的做法 / 我的方向」，落地时一并改；MCP 契约 5.2 与授权页也跟这一套，不允许两套并存。
...
**目标态明确写死这条：个人本体规模 ≤ 1e4 条，词面 + 时间衰减够用；向量只服务资料检索，不进入本体。** 这直接支撑 Q1（一个数据文件夹，可拷走），因为没有向量库就没有需要重建的索引。
...
| **新增分区的外发** | **进 `在线模型可见()` 与 `本地提示词可见()`，永不进 `可带走()`**（6.3）。MCP、上下文包、USER.md 投影与 `takeaway` 导出包拿到的画像里这两段根本不渲染；`full` 备份包含（6.3 ④） | 新增 |
...
这与产品前提 Q2 直接冲突。Q2 说资料只作为理解你的证据，理解本身是你的；但实现说「碰过资料的理解一律不许走」。
...
### 6.3 「出门」是三件不同的事，外加一件不算出门的（2026-09-21 新增，最重要的一条）

定位修正之前，「出门」是一个概念。之后它必须拆开，混为一谈会直接毁掉产品前提：

| | ① `可带走()` | ② `在线模型可见()` | ③ `本地提示词可见()` | ④ 全量备份 |
|---|---|---|---|---|
| 去哪 | **其他 AI 或其他软件**：MCP、上下文包、USER.md 投影、导出包的 `takeaway` 范围 | **知君自己调用的在线模型**（BYOK） | **只进本机模型的提示词**，不出这台电脑 | **用户自己的另一份存储**：`.zhijun` 包的 `full` 范围、系统备份 |
| 目的 | 让别的工具用上这份理解 | 让知君在在线模式下能当知己与导师 | 同上，但一个字不出门 | 让用户带走自己的东西 |
| `burdens` / `self_view` | **永不，且无开关** | **可见**（否则在线模式下知君根本无法承担这个角色） | **可见** | **在内**（见下） |
| 敏感 / 受限 | 永不 | 永不 | **可见** | 在内 |
| 未确认 `working` | 永不 | 永不 | **可见，标「待验证」** | 在内 |
| 用户的出路 | 无需出路，本来就不出去 | 切到本机模型（PRD 8） | 已经是最保守的一层 | 包要加密（未决 #10） |

**②③ 的差别是本次审查暴露出来的，过去被「模型可见」一个词盖住了。** 敏感内容与未确认候选**可以**进本机模型的提示词，**不可以**进在线模型——这正是「本地模式会记得更少、但什么都不出去」这句话的技术含义。

**④ 全量备份不是出门，因此不受 `可带走()` 约束。** 用户把自己的数据拷到自己的移动硬盘，接收方是用户本人，不是另一个 AI。C7 说的是「到达其他 AI 或其他软件」，备份两者都不是。但它必须满足三个条件，否则就是一个伪装成备份的泄漏面：

1. 界面上**不与「分享」「导出给别的工具」同一个按钮**，措辞是「备份 / 换一台电脑」。
2. 整包加密，密钥由用户口令派生（与未决 #10、PRD 未决 7 一起定）。
3. `manifest.json` 里 `scopeOfExport=full` 必须显著标注「**含只有你和知君看得到的内容**」。

这四层必须对用户明说，不能含糊（PRD 8 节）。今天 `core_profile.py:126` 的 `externalOk` 对应第 ② 层，第 6 节其余部分讲的全部是第 ① 层。
...
分区默认 = {who:on, principles:on, ways:on, direction:on,   # 「四开两关」待拍板，见契约 U6
            people:off, matters:off,
            burdens:永不, self_view:永不}      # 不是 off，是不可设置；这一行不待拍板（C7）
```
...
在线模型可见(c) =
  ...
  ∧ c.scope == long_term
  ∧ ¬本地血统(c)
  # 不含 section 与带走开关的判断：burdens / self_view 在这里可见

本地提示词可见(c) =
    c.trust_state ∈ {confirmed, working}          # working 要标「待验证」
  ∧ c.privacy ∈ {public, private, sensitive}      # restricted 仍然不进
  ∧ (c.scope == long_term ∨ 同一情境(c))
  # 不含 section、带走开关与本地血统的判断：本来就不出这台电脑
```

四个带走通道**必须共用 `可带走()`**，不得各自实现。在线画像装配用 `在线模型可见()`，本机提示词装配用 `本地提示词可见()`。**三个函数放在同一个模块里，不允许任何调用方自己拼条件。**
...
| 规则 | 判据 | 说明 |
|---|---|---|
| **内观分区永不** | `section ∈ {burdens, self_view}` | **新增。** 无开关，不可绕过。**只对 `可带走()` 生效**，不影响 ②③④ 三层（6.3） |
...
**列的口径**（这是上一版这张表最大的问题，必须先说清）：四个带走通道（界面导出的 `takeaway` 范围、USER.md 投影、上下文包、MCP）行为完全一致，因为它们共用 `可带走()`，所以合并成一列。在线画像与本地提示词是另外两个函数，各占一列。全量备份不是出门，单独一列。

| 输入（一条理解） | ① 四个带走通道 `可带走()` | ② 在线画像 | ③ 本地提示词 | ④ 全量备份 |
|---|---|---|---|---|
| `who`，confirmed，private，开关 unset | **是**（分区默认 on） | 进 | 进 | 在内 |
| `people`，confirmed，private，开关 unset | 否（分区默认 off） | 进 | 进 | 在内 |
| `people`，confirmed，private，开关 **on** | **是** | 进 | 进 | 在内 |
| `matters`，confirmed，private，开关 unset | 否 | 进 | 进 | 在内 |
| **`burdens` / `self_view`，confirmed，private** | **否（永不，无开关）** | **进** | **进** | **在内** |
| 任意分区，confirmed，**sensitive** | 否 | **不进** | **进** | 在内 |
| 任意分区，confirmed，**restricted** | 否 | 不进 | **不进** | 在内 |
| 任意分区，**working** | 否 | **不进** | **进，标「待验证」** | 在内 |
| 任意分区，**retracted / superseded** | 否 | 不进 | 不进 | **墓碑在内，正文不在** |
| 任意分区，`scope=context_only` | 否 | 不进 | 仅同一情境 | 在内 |
| 曾被设 `off` 的任意条 | 否（永久） | 进 | 进 | 在内 |

三行值得单独指出：

- **`burdens` / `self_view` 那一行**是定位修正带来的新行为，也是全表唯一一条「用户无法通过任何开关改变」的。它保护的是袒露本身的前提（6.3、6.5）。
- **`sensitive` 那一行**说明了「本地模式会记得更少」反过来也成立：本机模型其实看得到更多。这是本地模式的真实卖点，要在界面上讲清楚，别让用户以为本地只是个降级选项。
- **「曾被设 `off`」那一行**只影响带走，不影响知君自己用。用户说「这条别给别人看」，不等于说「这条你也别记得」。

「不进」与「进」都只描述内容是否出现在对应的提示词或载荷里，不涉及是否留回执（6.8）。
...
| `takeaway_receipts.jsonl` | 每行一条回执 | 第 6.8 节的出门记录 |
...
| `scopeOfExport` | enum | `full`（**全量备份，含 `burdens` / `self_view` 与敏感内容，必须加密**）/ `takeaway`（只含 `可带走()` 放行的内容，可以直接给别人）。两者在界面上是两个不同的动作，不共用按钮（6.3 ④） |
...
| `card_*`（知识卡片） | 9 | 与 claims 完全不相连，且占据了搜索的第一层，与 Q2 冲突 |
...
| 10 | **全量备份包的加密方式** | ① 用户口令派生密钥；② 系统钥匙串；③ 不加密 | **不是「要不要进包」——那个已在 6.3 ④ 定了：`full` 进、`takeaway` 不进。** 这里只剩加密方式，且必须与 PRD 未决 7（API key 的存储）用同一套方案，否则用户要记两个东西。③ 不可接受：一个含 `burdens` 的明文包被误发一次就够了 |
