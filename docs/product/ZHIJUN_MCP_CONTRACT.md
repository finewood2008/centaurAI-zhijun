...
知君被重新定位为**跑在任意一台电脑上的、独立的个人知识库**。它的主输出口是 MCP：任何 AI 工具（Claude Desktop、Claude Code、Cursor、自建 agent）在用户同意下，可以读到「这个人是谁」。

以下五条是不可违反的约束。任何实现、任何排期压力都不能绕过。
...
这五条同时是验收口径：任何一个 PR 若削弱其中一条，即便测试全绿也应当被拒。
...
知君积累的是「理解」，不是「文件」。理解如果只能在知君自己的界面里看，它的价值上限就是一个漂亮的日记本。用户真正的工作发生在 Claude Desktop、Cursor、终端里的 agent 中。知君要有用，必须让这些工具在**用户同意**的前提下读到「这个人是谁」，从而少问一轮、少猜一次、少写一段不像他的话。
...
| `get_section` | 取某个分区的全部可读条目 | `section` enum(6)；`limit` int 1..100（默认 30） | 该分区已授权 | 60 次/小时 | `SECTION_NOT_GRANTED` |
...
| `propose` | 提交一条提议，落入待确认 | `content` str 1..500；`section` enum(6)；`rationale` str 0..300 | 独立开关，默认关 | **10 次/小时，20 条/天** | `PROPOSAL_RATE_LIMITED` |
...
六个分区的键与标题今天已固定：`who / people / matters / principles / ways / direction`（`backend/mindos/agent/mcp_server.py:356`），对应「我是谁 / 我的人 / 我的事 / 我的原则 / 我的做法 / 我的方向」（`backend/zhijun_mcp/browser.py:18`）。
...
**与今天的差异**：今天授权页六个 checkbox 全部不勾（`backend/zhijun_mcp/browser.py:98-100`），默认全关。改为「四开两关」是行为变更，必须在授权页上以视觉方式呈现（已勾的四项仍可取消），并在首次连接时明确告知。这属于 `主张`，需要 johny 拍板确认默认值本身。
...
*附录 B 完。本文描述的现状均以本仓库 `newzhijun` 分支（`98c2331`）为准。*
