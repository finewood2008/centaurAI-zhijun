...
默认视图是一张图——**本体全景**：中心是「我」印；六个分区像扇面围成一圈（我是谁 · 我的人 · 我的事 · 我的原则 · 我的做法 · 我的方向）；三层环表达信任——实线内环是你确认过的、点线环是很久没再提需重申的、朱砂虚线是信任边界，边界外悬着的空心点是知君的猜测，等你点头才进来。点的颜色说层：墨色=你告诉我的，绿=资料里看到的，朱砂=你想成为的，空心虚线=我推测的；多处提到的有光晕，可带走的有小勾。点一个点在侧栏看细节并直接确认 / 修正 / 撤回；点分区名只看那一片。列表视图作为第二视图保留（六个抽屉）+ **知君最近学到的**（待确认的工作理解，取代原来的治理工作台）+ **需要你裁决**。空状态下同一张图配三句解释「靠近中心：你确认过的 / 点线环：很久没再提 / 朱砂线外：知君的猜测」，建档卡片里也用同一张小图解释什么是本体。
