...
「长期思考伙伴」是产品身份；「良师益友」只描述可测试的行为方式，不代表权威、情感或排他关系。
...
默认视图是一张图——**本体全景**：中心是「我」印；六个分区像扇面围成一圈（我是谁 · 我的人 · 我的事 · 我的原则 · 我的做法 · 我的方向）；三层环表达信任——实线内环是我确认过的、点线环是很久没再提需重申的、朱砂虚线是信任边界，边界外悬着的空心点是知君的推测，等我确认才进来。点的颜色说层：墨色=亲口陈述，绿=资料观察，朱砂=理想方向，空心虚线=知君推测；多处提到的有光晕，可带走的有小勾。点一个点在侧栏看细节并直接确认 / 修正 / 撤回；点分区名只看那一片。列表视图作为第二视图保留（六个抽屉）+ **知君最近学到的**（待确认的工作理解，取代原来的治理工作台）+ **需要你裁决**。空状态下同一张图配三句解释「靠近中心：我确认过的 / 点线环：很久没再提 / 朱砂线外：知君的推测」，建档卡片里也用同一张小图解释什么是本体。
...
## 6.1 「长」的五个机制（2026-09-02 晚，真实模型评测后补上）

真实模型评测暴露出它更像记录员：商量时不拉历史判断、章程里的挑战方式没被用上、复盘经验不会变成原则候选、承诺从不被回访、建档结束没有「第一次被理解」。补上五个机制：
1. **过去的判断进商量**：商量与回访时把用户自己记下的相似判断（标题 / 选择 / 结果 / 经验）带进上下文，知君先点出「这和你上次……很像」；原则与做法分区的已确认理解无论词面是否命中都带上。
2. **复盘经验 → 原则候选**：每条复盘写下的经验变成「你想成为的」待确认理解；两次复盘写下相近经验就置顶提示。
3. **承诺回访**：抽取器把「三个月内」「下周五前」换算成期限；到期前一天与当天提醒，措辞带「不想聊也可以先划掉」。
4. **第一次观察**：建档问完后，知君把至少两件事连起来给出恰好一条【我推测的】做事模式，以「对吗？」结尾，等用户点头。
5. **每周回顾**：周日一次，本周有 ≥3 条新确认理解或 ≥1 个判断才发，顺带点出一处张力；不含打卡语言。

人格也改了：先接住再追问；三种情况可以主动深入（不可逆的决定、与原则或过去判断不一致、用户要求）；敢挑战但只用已确认的理解和他自己的判断；看法要写理由与前提；把现在和过去连起来。模型永远不得宣称「已记进判断簿」，记录由面板完成。
...
| 我的本体 | `/me`、`/me/inbox` | 本体全景（默认）/ 列表 + 知君最近学到的 + 需要你裁决 |
| 判断 | `/judgments` | 判断簿（章程 / 判断 / 结果 / 复盘），每个判断带「记下当时 → 等结果 → 记下结果 → 复盘」步骤条 |
...
| Ollama / OpenAI 兼容 / Anthropic 通道 | 已用 DeepSeek（v4-pro 回复 / v4-flash 后台 JSON）跑通真实模型评测（`docs/development/real-model-eval.md`）：回复有标签、一次一问；发现推理型模型的 JSON 任务需要 ≥6000 token 预算，草稿改为后台任务 |
...
## 16. 2026-09-02 晚 · 全应用审视后的落地清单

三份审视（UI 质感 25 项、交互简洁 30 项、良师有效性）之后，按「质感 / 简单但可深入 / 真正帮到人」三个目标做的改动：

**质感（高端、克制）**
- 令牌：卡片改白、页面米纸、次级面淡米；标题宋体三级字阶；方印式标签（墨 = 你告诉我的、青绿 = 资料里看到的、朱砂虚线 = 我推测的、朱砂实线 = 你想成为的）；全库清掉企业蓝残留、800 字重、10/11px 小字。
- 顶栏只剩菜单与宋体标题；侧栏四入口 + 底部「偏好」；空白态改小印「空」。

**简单，但可深入（默认 / 高级三层）**
- 对话：起手三张卡（我在考虑一件事 / 最近发生了… / 你怎么看我？）；「深入」「我在考虑…」是两枚开关；候选理解只有「对 / 不对」+ ··· 菜单（部分对 / 只适用于这件事 / 先别存，每项带解释）；出处条讲人话（「参考了 3 条你确认过的理解、1 条还没点头的」）；判断草稿面板两段式「知君听到的 → 只有你能填的」，把握用滑杆。
- 本体：默认全景图，筛选收进按钮；每条理解默认一句话 + 来源一行 + 动作，「详情」才展开信任态、谓词、证据、时间；已确认动作改为「还是这样 / 不再这样了 / 修正」。
- 判断：记判断两步（这件事 → 为什么）；卡片默认只露标题、步骤条、当时选了、把握 / 回访。
- 资料与边界：两张卡（原材料 / 偏好）+ 一段边界说明；知识档案、回收站、搜索、导出、投影、删除全部记忆收进「高级」。
- 偏好：关系设置（提醒开关、每日上限）→ 模型与隐私（外部开关、提供商；回退 / 超时收进「更多」）→ 高级 · 运行时管理台（折叠）。

**真正帮到人（见 §6.1 五个机制）**：过去的判断进商量、复盘经验成原则候选、承诺回访、第一次观察、每周回顾；人格改为先接住不复述、敢挑战、看法带前提、不自称已记录。DeepSeek 三轮真实评测记录在 `docs/development/real-model-eval-notes.md`。

**明确没做的**
- `/today` 路由与 HomePage 未删（旧测试仍断言它），侧栏无入口等于隐藏。
- 本体全景没有紧凑版（侧栏宽度下会切掉边界点）；只做了标准版留边。
- 截图只有桌面视口（本机 Playwright 只能用系统 Chrome，平板 / 手机视口起不来）；旧页面（知识档案 / 原材料 / 搜索 / 回收站）只做了颜色字重的机械替换，没有重排。
- 真实模型评测只跑了一套 12 轮脚本，没有多人多天的使用数据；整合器的合并提案在真实抽取粒度下还没触发过。

## 17. 2026-09-03 · 外部评审之后：七个「第一天必撞」的缺陷与评审 P0 的落地形态

外部评审（对 9aa4e8e）的总判断成立：底层闭环在，但用户看不见「知君今天替我留下了什么」。逐条核对后，先修评审没提、但读代码就能确定的七个缺陷，再按本产品的原则落地它的 P0：

**第一批 · 确定性缺陷**
1. 建档入口一次性且可绕过 → intro 判定不再依赖统计接口成败；intro 态直接打字即进入建档；未完成的建档在空白态有「继续建档 · 还差 N 问」卡，会话列表标「建档 · 第 N 问」；起手卡与 intro 不再闪烁切换。
2. 切页截断正在生成的回复 → 离开页面不再中断流，只有显式「停止」才中断；建档收尾「去核对」只在正文写完后出现，并落到待确认列表而不是全景图。
3. 真实模型下候选理解 30 秒窗口静默丢失 → 轮询放到 120 秒；超时显示「整理好会出现在“知君最近学到的”」；后台任务清零时把晚到的候选挂回最近三条回复；抽取被跳过时给出原因（太短 / 在问知君 / 已暂停）。
4. 模型未配置时首句被吞 → 页头「还没配置模型 · 去偏好」，输入区禁用；首个 token 之前失败的发送把原文放回输入框。
5. 回访路径断成两半 → 同一判断只有一个回访会话（后端复用）；知君先开口（模板开场，不调模型）；记下结果后在同一面板复盘，经验直接进原则候选。
6. 建档第 7 问的静默领域不进提醒 → 提醒扫描同时读章程与本体里的边界理解（规则式抽词）。
7. 提醒把人带离对话 → 「原则与做法」「承诺回访」的主动作把问句放进输入框，次要链接才去看理解。

**第二批 · 评审 P0 的落地形态**
- 「今日 / 下一步」不做第五入口、不恢复旧 HomePage（已删除 /today）：对话页标题下一行灰字聚合「待确认 N · 待回访 N（逾期 N）· 待复盘 N · 还在整理 N 件」，空白态起手卡上方有「下一步」面板（最多 3 条，措辞不催）。
- 「成果回执」不做离开前弹层：消息流底部一张安静的「这段对话留下的」卡（已确认 / 等你点头 / 判断入簿与回访日 / 承诺期限 / 还在整理）；会话列表每条带模式方印与产出一行。
- 依据显式化：出处条写「参考了你记下的「…」（当时选了「…」）」与「和你的原则「…」有关」；普通聊天也带相似判断；不做「这次改变什么」（那要模型断言自己的影响）。
- 快改：首句结果导向不写时长；判断页先卡片后趋势（少于 5 条默认折叠）、图例三项；本体图默认不显示节点名、只留信任边界一条环标签、扇区不带计数、图例折叠、图内最小 12px；资料页「高级」用名字不用数量。

**第三批 · 待执行**：7 天真实使用协议见 `docs/development/real-model-eval-notes.md`。在这份数据出来前不再加新面板。

**仍未做**：出设备回执没有存历史判断与锚点（刷新后由回执还原的出处这两项为空）；静默词抽取是规则式；「这段对话留下的」只在本次聊完后出现，重开旧会话不显示；到期承诺靠前端过滤最近 500 条理解；截图仍只有桌面视口。

## 18. 2026-09-03 · 产品负责人拍板：「今日」为首屏，对话回归简洁

负责人判断：把提醒、下一步、聚合行都堆在「对话」里是不对的，对话应该是一个相对简洁的板块；应有一个「今日」板块作为用户打开应用的首屏，把这些信息放到今日里呈现。此决定覆盖 §17 里「不做第五入口」的做法。

信息架构改为五入口：**今日来信（/）· 对话（/chat 空白态，/c/:id 会话）· 我的本体 · 判断 · 资料与边界**，底部偏好。

「今日」自上而下（全部安静、无红点、无计数徽章；没内容的区块不渲染）：
1. 问候一行：称呼 + 日期（模板，不调模型），下一行一句按数据拼的话（「有 N 件事到了回访的时候 · N 条理解等你点头」，全零则「今天没有要催你的事。」）。
2. 知君想跟你聊的：今日提醒（≤3，带为何现在；回访开回访会话，其它把话头带到对话）。
3. 下一步：到期回访 / 待复盘 / 到期承诺 / 等你点头（≤3）。
4. 最近留下的：最近三段有产出的对话（模式方印 · 标题 · 产出一行）。
5. 带一件事来：三张起手卡 → 对话页并预填。
6. 未建档时整页只有问候 + 「先让我认识真实的你」；建档未完成时有「继续建档 · 还差 N 问」。

「对话」只剩：会话列表（模式方印 + 产出行）、消息流（候选理解、出处条）、输入框、商量时的草稿面板、回访面板、「这段对话留下的」卡；页头只保留模型名与「还在整理 N 件事」。
