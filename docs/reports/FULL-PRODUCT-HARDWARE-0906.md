...
资料能力最近一轮针对性回归为 189 项通过；网关集合、桌面和其他模块的批次分别见执行计划，不累计为单次全链路通过数。额外扩展运行旧 E3 消费者测试时有 158 项通过、11 项因外置 `docs/development/data-engine-e0/contracts.schema.json` 缺失而失败：测试按 worktree 外层父目录寻找该合同，当前工作区未提供。未伪造该合同或把这些失败改记为通过，因此不声称整个 DE 仓库测试套件全部通过。
