...
因此，这些本地代码之间尚未形成可证实的正式鉴权闭环。真实部署可能有其他组件，但本轮没有登录设备核验，不能把这种可能性当成已解决。

### 3.2 推荐解决方向与验收

建议由**盒端可信身份桥**把已验证的连接主体映射为 data-engine 可验证的业务上下文，或由主进程获取专用 MindOS ticket 并经批准的 Agent 内部握手完成交换。两者选其一，明确 issuer/audience、设备/账号/应用/scope 绑定、有效期、nonce、续期、撤销与重连规则。
...
这里没有指定可以复用某张现有 JWT：控制面合同和 Agent/服务端改动范围需先确认。禁止通过 renderer 传入自称可信的身份头，也不能用开启 `MINDOS_LOCAL_WEB_DEBUG_ACCESS` 或取消 loopback/session guard 作为正式方案。
