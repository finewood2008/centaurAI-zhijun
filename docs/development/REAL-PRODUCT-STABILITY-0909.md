...
- 所需容器 ID、RestartCount 不变，`OOMKilled=false` 且保持 running。
...
当前 CentaurOS 资源合同仍有两个验收缺口：Docker 证据没有容器 PID，资源命令也没有 Ollama 日志中的实际 compute backend。工具能检查容器 ID/restart/OOM 和 Ollama systemd PID，但不能据此宣称 `cpu_avx2` 已实际执行。正式解除限流前仍需由 CentaurOS 资源证据扩展这两个字段，并在 runner 中加入强制比对。
