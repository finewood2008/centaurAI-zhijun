...
日期：2026-09-06（文件名沿用 0905 集成基线）。规划代码基线：知君 `140dd34`，产品源 `22dc9a3`。本轮只生成开发任务，**所有开发任务均未开始**；不将既有产品回归、接口样例或本文勾选项视为业务集成完成。
...
影响文件：本文、`tasks/` 下三份明细及 README/架构/集成/工作包导航。验收标准是任务能够直接分配给开发者，依赖可追踪，实施步骤与正负向验收具体，未知配置有责任方，模拟测试和真机测试分别记录。本轮不创建远程 Issue、不改应用代码、依赖、相邻仓库或运行数据。
...
**文件与交付：** 当前各仓 `package.json`、lockfile、SDK release manifest；拟新增 `docs/development/integration-release-baseline.json`。记录源码 SHA、相关 dirty 改动的可追溯交付、SDK tgz/sidecar 哈希、协议与 OS/CPU 范围，不写凭据。
...
- [ ] 明确现有 shell Electron 33 与 SDK 验证环境 Electron 37 的兼容验证策略，不仅按版本号推断可用。
...
共 **66 项**：公共前置5项、桌面与前端23项、服务端20项、连接与发布18项（其中2项为可选BLE）。以下索引链接到具体步骤，状态均为未开始；负责人和依赖详见正文。
...
| 顺序 | 可派发任务 | 输出 / 转入下一批条件 |
| --- | --- | --- |
| 现在开始 | BASE-01、BASE-05；SERV-04同步准备只读调查 | 本地开发版本、隔离规范和合成测试数据；正式版本清单继续补齐 |
| 本地桌面基础 | DESK-01→02→03，随后并行DESK-09/10、12 | 独立入口、窄IPC、状态机、草案资料策略；不等正式账号 |
| 本地资料合流 | DESK-11、13→14；按依赖可先做DESK-18 | M0-L通过，关闭由fake端口验证，正式清理另验 |
...
本轮仅检查文档链接、任务 ID 唯一性、依赖可解析与无环、WP-00–WP-10 覆盖、实施/验收字段和当前事实一致性，不运行产品服务或业务回归。后续开发按任务增量验证；现有命令与隔离要求见工作包第4节，不能把尚不存在的测试脚本写成可执行入口。
...
交叉审核补齐了普通领域RPC的main侧适配、M1路由/建档守卫、新分页/幂等/清除的前端任务，以及附件到回复的M2关闭条件。所有开发复选框仍未勾选，本记录只说明任务文档通过检查。
