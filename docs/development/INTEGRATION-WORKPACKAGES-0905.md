...
> 更新：2026-09-06。状态：M0-L 本地实现与隔离验证已完成，Consumer真实登录/设备查询已验，签名桥三端已编码，尚未部署盒端或完成真实资料读取。原规划把 [集成方案](INTEGRATION-0905.md) 的 P0–P7 细化为可分工工作包；客户端接口以 [桌面接口合同](DESKTOP-CONTRACT-0905.md) 为准，部署边界以 [架构](ARCHITECTURE-0905.md) 为准。
...
| 正式应用身份、业务身份桥 | 已自动配置现有 PC 资料目标；macOS存储/原生sidecar通过；真实登录及设备查询通过；签名桥待盒端部署 | WP-00、WP-02、WP-03 的部署配置和正负向结果 |
...
D03后续交付已覆盖WP-03的Agent签发、DE验签和独立资料scope；WP-04通过同一SDK context握手后开放最小资料GET。三端源码、本地测试与可部署输入见[签名桥记录](BUSINESS-BRIDGE-0906.md)；WP-05真实Direct资料、跨主体及退出验证仍待盒端部署。
