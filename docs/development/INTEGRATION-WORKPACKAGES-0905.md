...
> 更新：2026-09-06。状态：M0-L 本地实现与隔离验证已完成，Consumer真实登录/设备查询已验，签名桥三端已编码，家中盒端已部署，真实SDK空资料页及一次断开/重连已验，完整矩阵待验。原规划把 [集成方案](INTEGRATION-0905.md) 的 P0–P7 细化为可分工工作包；客户端接口以 [桌面接口合同](DESKTOP-CONTRACT-0905.md) 为准，部署边界以 [架构](ARCHITECTURE-0905.md) 为准。
...
每个工作包的具体开发步骤、文件归属、依赖、估算及验收见 [详细开发任务清单](DEVELOPMENT-TASKS-0905.md)。本文保留 WP 级责任与完成定义，任务状态由详细清单维护。当前实现文件、命令与未验项见 [M0-L 实施记录](M0-IMPLEMENTATION-0906.md)；工作包的正式验收不能用本地模拟替代。
...
推荐先用 macOS ARM64 与已有绑定设备完成这一条链路；此为实施建议。M0 目标应用已采用三端登记的 `mindos-person-data-pc`，知君 clientId/密钥/存储独立。领域并入盒端 data-engine、业务权限、配额、平台和发布范围仍需落实；相邻仓库现有未提交文件不自动成为发布依赖。
...
| 桌面 host、模拟端口与隔离测试 | M0-L 已实现并完成本地验证 | 见实施记录；WP-01/04/05 的本地子交付使用 fake adapter，真实链路待验 |
| 正式应用身份、业务身份桥 | 已自动配置现有 PC 资料目标；macOS存储/原生sidecar通过；真实登录及设备查询通过；签名桥已部署家中盒子，正式负向检查通过 | WP-00、WP-02、WP-03 的部署配置和正负向结果 |
| SDK、sidecar、Agent、后端发布组合 | 尚未固定为可复现组合 | 提交/归档哈希、合同版本、实际二进制来源及兼容报告 |
| M0 正式设备验收 | 局部通过，完整闭环未完成 | WP-05 的真机记录，不以既有产品测试数量替代 |
...
- **影响文件**：本文件、桌面接口合同、`integration-release-baseline.json`、shell 受控产品配置与准备脚本；D03 按需修改 OS 现有 PC 目标清单及可信桥。真实密钥和 token 不入文档。
- **前置**：当前源码基线和只读资料范围已完成调查；现有 PC 目标与 Consumer/Gateway 已从三端产品配置和策略核定，不从 CLI 凭据推导授权。
- **交付物**：冻结 Consumer/Gateway 信任地址、目标 applicationId、独立 clientId/存储 namespace、purpose/scopes、目标路径/headers/限额、账号与设备归属、身份桥选择，以及 SDK/sidecar/Agent/data-engine 提交和实际产物哈希；记录变更负责人和兼容范围。
...
以下为既有产品基线命令。原工作包编写阶段没有执行它们；新增 M0 专用测试与本轮实际结果见实施记录，不能把旧产品回归当作真实 SDK 验证。
...
每个编码工作包应随实现新增自己的单元/组件/负向测试，并在 PR 中填写**实际存在的测试文件、执行命令、输出摘要和未通过项**。M0-L 现有 shell `test` / `test:e2e`、前端 `test:desktop`，执行目录见实施记录；`test:m0` 与流/上传专用测试仍未提供。新增测试只写临时状态；真实环境测试记录配置版本与关联 ID，避免保存票据、签名密钥和业务正文。
...
后续编码已交付正式密码登录、系统加密凭据、共享刷新、设备/票据和SDK装配，属于WP-02本地子交付；验证与D03剩余输入见[正式接入记录](M0-PRODUCTION-0906.md)。

D03后续交付已覆盖WP-03的Agent签发、DE验签和独立资料scope；WP-04通过同一SDK context握手后开放最小资料GET。三端源码、本地测试与可部署输入见[签名桥记录](BUSINESS-BRIDGE-0906.md)；WP-05真实Direct空页及一次断开重连已验；非空资料、退出后重新登录第二轮及跨主体矩阵仍待执行。
