...
状态：2026-09-06 三端代码及本地验证已完成，盒端部署和真实资料验收待完成。用户已在独立知君窗口登录；截图中的旧main在SDK连接前返回 `BUSINESS_BRIDGE_REQUIRED`，根因是未注入业务桥，不是密码或应用参数错误。
...
- [ ] 在有SSH认证的真实盒部署，验证当前账号资料读取和断开后拒绝；没有真实结果不关闭M0-R。
...
| data-engine | `dev/zhijun-business-bridge-0906` | `f6b1089`，基线 `ec2854e`；原dirty工作区未改 |

完整提交和SDK产物哈希保存在[版本基线](integration-release-baseline.json)。两个盒端分支已推送远程；提交存在不代表已部署。
...
这些是本地或合成验证。真实Consumer登录和两台在线授权设备列表已在用户窗口确认；真实Direct资料、跨账号/设备隔离及断开拒绝仍未验。
...
默认从两个隔离worktree构建，要求干净且HEAD与版本基线完全一致。也可传入三个绝对路径：OS工作区、DE工作区、新输出目录。父目录需已存在且不含软链接，已有输出不会覆盖。输出默认在忽略目录 `data/desktop/bridge-release-0906/`，含Linux AMD64/ARM64 Agent、两仓补丁、6个DE运行文件、应用清单参考及SHA256 manifest。该包不含盒内私钥、不执行SSH、不安装服务，不是签名OTA产物。

1. 获得有效SSH入口后，先只读核查盒子架构、实际deviceId、Agent/DE版本、systemd服务用户、ExecStart、配置/授权快照路径和数据库路径。当前`user@192.168.1.18`公钥认证失败，尚不能推定实际部署路径与服务权限。
...
4. 保持 `MINDOS_LOCAL_WEB_DEBUG_ACCESS` 关闭，验证配置后有序重启盒端服务，再重启新版知君并由用户重新登录。当前保留的已登录窗口仍执行旧main；只刷新renderer无法加载本次主进程修复。
...
尚需用户提供已有可用SSH主机别名，或用户名及密钥文件路径；不需要在聊天中发送密码、私钥或令牌。[真机记录](REAL-ACCEPTANCE-0906.md)在完成真实请求前保持M0-R未通过。
