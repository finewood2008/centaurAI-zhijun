...
更新日期：2026-09-13。范围：Electron 桌面端；不改变 Admin、Gateway 或盒端的短期授权时限。
...
连接页使用 `SecureConnectionProgress.vue` 展示三个真实阶段，不以动画或固定等待时间推进：

| 主进程状态 | 界面含义 | 尚未证明的事实 |
|---|---|---|
| `connecting` | 正在建立连接，优先直连，必要时尝试受控加密中继 | 此时还不知道最终 Direct/Relay 路径，也不表示访问权限已通过 |
| `authorizing` | 通道已建立，正在验证访问权限；按 `selectedPath` 展示实际路径 | 通道可达不表示资料访问或工作区授权成功 |
| `ready` 且支持 product | 连接及授权完成，准备打开工作区；产品作用域就绪后立即进入页面 | 不人为停留成功动画，不代替资料使用或在线外发授权 |
| `ready` 但不支持 product | 连接已建立，工作区服务尚未就绪 | 不把旧只读连接误报为完整知君可用 |
| `failed` / `disconnecting` | 连接未完成 / 正在结束连接 | 之前成功的步骤不作为当前安全可用证明 |

界面用单调时钟显示本次观察到的等待时长，8 秒、20 秒后补充等待说明和可选操作；不显示虚构百分比、预计完成时间、内部重试次数或当前中继尝试。计时不读取私有诊断日志，按账号、设备及连接代次重新开始；失败或组件卸载停止计时。读屏仅播报阶段变化，不逐秒播报计时；弱化动画偏好下关闭循环动画。

连接中的“取消连接”调用已有 `disconnect`，允许抢占仍在等待的 `connect`，但不抢占登录、退出或另一次断开。主进程清理当前会话、取消恢复并递增 generation；迟到结果不得恢复旧盒子或触发业务读取。取消是停止等待及撤销当前会话，不承诺远端所有异步工作立即停止。失败后保留公开错误和重新选择盒子的入口，不自动重放业务写请求。

工作区真正就绪后，顶栏仅在正式环境且已知实际通道时展示加密锁标识。直连与中继均解释为电脑到盒子的加密通道，不宣称无服务器参与、无元数据或在线模型内容永不外发；模拟环境明确不证明真实连接安全性。

验收命令（在 `frontend/mindos-web`）：`npm run test:desktop` 和 `npm run test:secure-connection`。后者构建桌面页面并用合成 IPC 在浏览器验证阶段、取消、隔离与响应式布局，不使用真实账号，不代表真实外网延迟已降低。此体验随下一次桌面安装包更新交付，不需要修改盒端协议、路由或授权期限。

2026-09-13 源码验收：前端 `test:all` 174/174、`test:desktop` 46/46（包含在前者中）通过；Shell runtime/reconnect 57/57 通过；桌面页面构建、产品导航和安全连接浏览器回归通过。浏览器覆盖 360/768/1440 宽度、真实阶段映射、模拟环境、迟到结果隔离、取消与加密标识；全部使用合成连接。这次只完成源码与前端构建，未生成新安装包、覆盖已安装客户端或更新盒端服务。

后续已按用户要求生成 0.1.19 正式 macOS ARM64 安装包，签名、DMG/ZIP 验包和隔离启动通过；未覆盖已安装客户端。详见 [0.1.19 发布记录](release-0.1.19-secure-connection.md)。
