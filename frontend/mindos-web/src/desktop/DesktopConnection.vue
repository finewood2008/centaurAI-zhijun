...
onMounted(() => { unsupportedRoute.value = false })
  ...
  <div class="desktop-shell">
    ...
    <main>
      ...
      <section v-else-if="unsupportedRoute" class="welcome-card">
        <p class="eyebrow">当前桌面版本</p><h1>此功能暂未开放</h1><p>请先完成登录并连接盒子。</p>
      ...
      <template v-else>
        ...
        <p v-if="phase === 'ready'" class="notice">盒子已连接，知君工作区尚未就绪。请重新连接以加载完整产品服务。</p>
      </template>
    </main>
  </div>
</template>
