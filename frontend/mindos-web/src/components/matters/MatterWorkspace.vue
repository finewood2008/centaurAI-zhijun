  ...
  <SideDrawer :open="open" title="事情与成果" @close="open = false">
    <div class="matter-workspace" :aria-busy="busy || loading">
      ...
      <template v-if="matter">
        ...
        <div class="actions"><button :disabled="busy || disabled" @click="review">一起回顾这件事</button><RouterLink v-if="matter.decisionId" :to="{ path: '/review', query: { decisionId: matter.decisionId } }">回看这次选择</RouterLink></div>
      </template>
