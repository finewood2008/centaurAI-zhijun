...
export async function prepareChatRoute(id: string, body: Record<string, unknown>): Promise<(Record<string, unknown> & { routeRevision: string }) | null> {
  ...
  for (let i = 0; i < 6; i++) {
    const preview = await routingRequest<RoutePreview>(routePath(id) + '/preview', 'POST', data)
    if (preview.charterConflict) {
      const choice = await askRoute(preview)
      ...
      if (choice.action === 'exception' && preview.charterConflict.canOverride) {
        const result = await routingRequest<{ exceptionId: string }>(routePath(id) + '/charter-exception', 'POST', { revision: preview.revision, exceptionKey: preview.charterConflict.exceptionKey, acknowledge: true })
    ...
    const choice = await askRoute(preview, true)
    if (choice.action === 'cancel') return null
    if (choice.action === 'local') data = { ...data, localOnly: true }
    else if (choice.action === 'omit') data = { ...data, omitSources: true }
    else await routingRequest(routePath(id) + '/grant', 'POST', { revision: preview.revision, keys: choice.keys })
  }
...
export async function routedTask<T>(id: string, path: string, body: object, signal?: AbortSignal): Promise<T> {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (!preview.service.external || !preview.missing.length) {
      return routingRequest<T>(path, 'POST', { ...data, routeRevision: preview.revision }, signal)
    }
    const choice = await askRoute(preview, false, signal)
