...
try {
  ...
  await page.getByText('先用小范围试验验证，再决定是否继续。', { exact: true }).waitFor()
  assert.equal((await page.locator('[data-message-id="persisted-assistant"] .zj-msg__body').textContent())?.trim(), '先用小范围试验验证，再决定是否继续。')
  ...
  await page.getByTestId('context-cited').getByText('一次可撤回的探索', { exact: false }).waitFor()
  assert.equal(await page.getByTestId('provenance-graph').getByText('[p2]', { exact: false }).count(), 0)
