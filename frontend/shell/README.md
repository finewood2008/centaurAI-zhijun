...
登录成功并处于选盒页面时，“扫描附近盒子”会打开独立的本地配网窗口。扫描必须由用户点击触发，候选设备也必须手动选择；正式 v2 流程会校验盒身 6 位码和 RSA 设备证书，再通过 AES-256-GCM 加密通道发送用户手工输入的 Wi-Fi 参数。P0 不远程扫描 Wi-Fi。蓝牙临时 ID 不会写成业务设备 ID，Wi-Fi 密码也不会进入主产品页面、主 IPC 或持久存储。关闭窗口、退出登录或切换身份时会断开本次配网会话。
